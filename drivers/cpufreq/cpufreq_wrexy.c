...
*	     (C)  2013 John Thatcher <jthatcher@temple.edu>
 *
...
static DEFINE_PER_CPU(struct wrexy_info_s, wrexy_info);

#define dprintk(flag,msg...) do { \
	if (debug_mask & flag) pr_info("[wrexy]" ":" msg); \
	} while (0)
...
enum {
	...
	WREXY_DEBUG_ALG = 4,
	WREXY_DEBUG_BOOST = 8,
	WREXY_DEBUG_INPUT = 16,
	WREXY_DEBUG_SUSPEND = 32
};

/*
 * Combination of the above debug flags.
 */
//static unsigned long debug_mask = WREXY_DEBUG_LOAD|WREXY_DEBUG_JUMPS|WREXY_DEBUG_ALG|WREXY_DEBUG_BOOST|WREXY_DEBUG_INPUT|WREXY_DEBUG_SUSPEND;
...
static struct workqueue_struct *wrexy_wq;
...
#ifdef CONFIG_HAS_EARLYSUSPEND
static struct early_suspend wrexy_early_suspend_handler;
#endif
...
static int cpufreq_governor_wrexy(struct cpufreq_policy *policy,
		unsigned int event);
...
struct cpufreq_governor cpufreq_gov_wrexy = { 
    .name = "wrexy", 
    .governor = cpufreq_governor_wrexy, 
...
#ifdef CONFIG_CPU_FREQ_GOV_WREXY_30
...
inline static void wrexy_update_min_max(
		struct wrexy_info_s *this_wrexy, struct cpufreq_policy *policy) {
	this_wrexy->ideal_speed = // ideal_freq; but make sure it obeys the policy min/max
...
inline static void wrexy_update_min_max_allcpus(void) {
	unsigned int cpu;

	for_each_online_cpu(cpu)
	{
		struct wrexy_info_s *this_wrexy = &per_cpu(wrexy_info, cpu);
		if (this_wrexy->cur_policy){
			if (lock_policy_rwsem_write(cpu) < 0)
				continue;

			wrexy_update_min_max(this_wrexy, this_wrexy->cur_policy);
...
static inline void dbs_timer_init(struct wrexy_info_s *this_wrexy) {
	int delay = get_timer_delay();

	INIT_DELAYED_WORK_DEFERRABLE(&this_wrexy->work, do_dbs_timer);
	schedule_delayed_work_on(this_wrexy->cpu, &this_wrexy->work, delay);
}

static inline void dbs_timer_exit(struct wrexy_info_s *this_wrexy) {
	cancel_delayed_work_sync(&this_wrexy->work);
}

inline static void target_freq(struct cpufreq_policy *policy,
		struct wrexy_info_s *this_wrexy, int new_freq, int old_freq,
		int prefered_relation) {
	int index, target;
	struct cpufreq_frequency_table *table = this_wrexy->freq_table;
	unsigned int cpu = this_wrexy->cpu;

	dprintk(WREXY_DEBUG_ALG, "%d: %s\n", old_freq, __func__);
	...
	if (!cpufreq_frequency_table_target(policy, table, new_freq,
		...
		dprintk(WREXY_DEBUG_ALG, "frequency change failed\n");
		return;
	}

	dprintk(WREXY_DEBUG_JUMPS, "%d: jumping to %d (%d) cpu %d\n", old_freq, new_freq, target, cpu);

	__cpufreq_driver_target(policy, target, prefered_relation);

	// remember last time we changed frequency
	this_wrexy->freq_change_time = ktime_to_us(ktime_get());
}

/* We use the same work function to sale up and down */
static void cpufreq_wrexy_freq_change(struct wrexy_info_s *this_wrexy) {
	unsigned int cpu;
	...
	ramp_dir = this_wrexy->ramp_dir;
	old_freq = this_wrexy->old_freq;
	policy = this_wrexy->cur_policy;
	cpu = this_wrexy->cpu;
	...
	if (old_freq != policy->cur) {
		...
		dprintk(WREXY_DEBUG_ALG, "%d: frequency changed by 3rd party to %d\n",
				old_freq, policy->cur);
		new_freq = old_freq;
	} else if (ramp_dir > 0 && nr_running() > 1) {
		// ramp up logic:
		if (old_freq < this_wrexy->ideal_speed)
			new_freq = this_wrexy->ideal_speed;
		...
		if (old_freq > this_wrexy->ideal_speed) {
			...
			new_freq = old_freq * this_wrexy->cur_cpu_load / max_cpu_load;
	...
	if (new_freq!=0){
		target_freq(policy, this_wrexy, new_freq, old_freq, relation);
	}
	
	this_wrexy->ramp_dir = 0;
}

static inline void cpufreq_wrexy_get_ramp_direction(struct wrexy_info_s *this_wrexy, u64 now)
{
	unsigned int cur_load = this_wrexy->cur_cpu_load;
	unsigned int cur = this_wrexy->old_freq;
	struct cpufreq_policy *policy = this_wrexy->cur_policy;
	
	// Scale up if load is above max or if there where no idle cycles since coming out of idle,
	// additionally, if we are at or above the ideal_speed, verify we have been at this frequency
	// for at least up_rate:
	if (cur_load > max_cpu_load && cur < policy->max
			&& (cur < this_wrexy->ideal_speed
				|| (now - this_wrexy->freq_change_time) >= up_rate)) {
		dprintk(WREXY_DEBUG_ALG,
				"%d: ramp up: load %d\n", cur, cur_load);
		this_wrexy->ramp_dir = 1;
	}
	...
	// frequency we require that we have been at this frequency for at least down_rate:
			...
			&& (cur > this_wrexy->ideal_speed
				|| (now - this_wrexy->freq_change_time) >= down_rate)) {
		dprintk(WREXY_DEBUG_ALG,
				"%d: ramp down: load %d\n", cur, cur_load);
		this_wrexy->ramp_dir = -1;
	}
...
static void cpufreq_wrexy_timer(struct wrexy_info_s *this_wrexy) {
	unsigned int cur;
	struct cpufreq_policy *policy = this_wrexy->cur_policy;
	...
	timer_stat[cpu] = now;
	printk(KERN_DEBUG "[wrexy]:cpu %d %lld\n", cpu, diff);
#endif

	cur = policy->cur;

	dprintk(WREXY_DEBUG_ALG, "%d: %s cpu %d %lld\n", cur, __func__, cpu, now);

	for_each_cpu(j, policy->cpus)
	{
		struct wrexy_info_s *j_this_wrexy;
		u64 cur_wall_time, cur_idle_time, cur_iowait_time;
		unsigned int idle_time, wall_time, iowait_time;
		unsigned int cur_load;
		
		j_this_wrexy = &per_cpu(wrexy_info, j);

		cur_idle_time = get_cpu_idle_time(j, &cur_wall_time);
		cur_iowait_time = get_cpu_iowait_time(j, &cur_wall_time);

		wall_time = cur_wall_time - j_this_wrexy->prev_cpu_wall;
		j_this_wrexy->prev_cpu_wall = cur_wall_time;

		idle_time = cur_idle_time - j_this_wrexy->prev_cpu_idle;
		j_this_wrexy->prev_cpu_idle = cur_idle_time;

		iowait_time = cur_iowait_time - j_this_wrexy->prev_cpu_iowait;
		j_this_wrexy->prev_cpu_iowait = cur_iowait_time;
...
#ifdef CONFIG_CPU_FREQ_GOV_WREXY_30
			cur_nice = kstat_cpu(j).cpustat.nice - j_this_wrexy->prev_cpu_nice;
			cur_nice_jiffies = (unsigned long) cputime64_to_jiffies64(cur_nice);

			j_this_wrexy->prev_cpu_nice = kstat_cpu(j).cpustat.nice;
#else
			cur_nice = kcpustat_cpu(j).cpustat[CPUTIME_NICE] - j_this_wrexy->prev_cpu_nice;
			cur_nice_jiffies = (unsigned long) cputime64_to_jiffies64(cur_nice);

			j_this_wrexy->prev_cpu_nice = kcpustat_cpu(j).cpustat[CPUTIME_NICE];
		...
		j_this_wrexy->cur_cpu_load = cur_load;
	}

	/* calculate the scaled load across CPU */
	load_at_max_freq = (this_wrexy->cur_cpu_load * policy->cur)/policy->cpuinfo.max_freq;

	cpufreq_notify_utilization(policy, load_at_max_freq);

	dprintk(WREXY_DEBUG_LOAD, "%d: load %d\n", cpu, this_wrexy->cur_cpu_load);

	this_wrexy->old_freq = cur;
	this_wrexy->ramp_dir = 0;

	cpufreq_wrexy_get_ramp_direction(this_wrexy, now);

	// no changes
	if (this_wrexy->ramp_dir == 0)		
		return;

	// boost - but not block ramp up steps based on load if requested
	if (boost_running){
		if (now < boost_end_time) {
			dprintk(WREXY_DEBUG_BOOST, "%d: cpu %d boost running %llu %llu\n", cur, cpu, now, boost_end_time);
		
			if (this_wrexy->ramp_dir == -1)
				return;
			else {
				if (ramp_up_during_boost)
					dprintk(WREXY_DEBUG_BOOST, "%d: cpu %d boost running but ramp_up above boost freq requested\n", cur, cpu);
				else
					return;
			}
		} else
	...
	cpufreq_wrexy_freq_change(this_wrexy);
}

static void do_dbs_timer(struct work_struct *work) {
	struct wrexy_info_s *this_wrexy =
			container_of(work, struct wrexy_info_s, work.work);
	unsigned int cpu = this_wrexy->cpu;
	int delay = get_timer_delay();

	mutex_lock(&this_wrexy->timer_mutex);

	cpufreq_wrexy_timer(this_wrexy);

	queue_delayed_work_on(cpu, wrexy_wq, &this_wrexy->work, delay);
	mutex_unlock(&this_wrexy->timer_mutex);
}
...
static void update_idle_time(bool online) {
	...
	for_each_possible_cpu(j)
	{
		...
		j_this_wrexy->prev_cpu_idle = get_cpu_idle_time(j,
				&j_this_wrexy->prev_cpu_wall);
...
static struct attribute * wrexy_attributes[] = { 
...
static struct attribute_group wrexy_attr_group = { 
	.attrs = wrexy_attributes, 
	.name = "wrexy", 
	};

static int cpufreq_wrexy_boost_task(void *data) {
	struct wrexy_info_s *this_wrexy;
	u64 now;
	struct cpufreq_policy *policy;
#ifndef CONFIG_CPU_FREQ_GOV_WREXY_TEGRA
	unsigned int cpu;
	...
	while (1) {
        ...
        this_wrexy->prev_cpu_idle = get_cpu_idle_time(0,
						&this_wrexy->prev_cpu_wall);

        unlock_policy_rwsem_write(0);
#else		
		for_each_online_cpu(cpu){
			this_wrexy = &per_cpu(wrexy_info, cpu);
			if (!this_wrexy)
				continue;
			...
			if (policy->cur < cur_boost_freq) {
				start_boost = true;
				dprintk(WREXY_DEBUG_BOOST, "input boost cpu %d to %d\n", cpu, cur_boost_freq);
				target_freq(policy, this_wrexy, cur_boost_freq, this_wrexy->old_freq, CPUFREQ_RELATION_H);
				this_wrexy->prev_cpu_idle = get_cpu_idle_time(cpu, &this_wrexy->prev_cpu_wall);
			}
			mutex_unlock(&this_wrexy->timer_mutex);
		...
		dprintk(WREXY_DEBUG_BOOST, "%s %llu %llu\n", __func__, now, boost_end_time);
	...
	pr_info("[wrexy]:" "%s boost_thread stopped\n", __func__);
	return 0;
}

#ifdef CONFIG_INPUT_MEDIATOR

static void wrexy_input_event(struct input_handle *handle, unsigned int type,
...
static struct input_mediator_handler wrexy_input_mediator_handler = {
	.event = wrexy_input_event,
	};
...
static int dbs_input_connect(struct input_handler *handler,
	...
	pr_info("[wrexy]:" "%s input connect to %s\n", __func__, dev->name);
	...
	err2: kfree(handle);
	pr_err("[wrexy]:" "%s faild to connect input handler %d\n", __func__, error);
	return error;
}
...
#ifdef CONFIG_HAS_EARLYSUSPEND
static void wrexy_early_suspend(struct early_suspend *h)
{
	dprintk(WREXY_DEBUG_SUSPEND, "%s\n", __func__);
	ideal_freq = suspend_ideal_freq;
	is_suspended = true;
	wrexy_update_min_max_allcpus();
}

static void wrexy_late_resume(struct early_suspend *h)
{
	dprintk(WREXY_DEBUG_SUSPEND, "%s\n", __func__);
	ideal_freq = awake_ideal_freq;
	is_suspended = false;
...
static int cpufreq_governor_wrexy(struct cpufreq_policy *new_policy,
		unsigned int event) {
	...
	case CPUFREQ_GOV_START:
		...
		this_wrexy->cur_policy = new_policy;
		this_wrexy->cpu = cpu;

		wrexy_update_min_max(this_wrexy,new_policy);

		this_wrexy->freq_table = cpufreq_frequency_get_table(cpu);
		if (!this_wrexy->freq_table){
		...
		if (dbs_enable == 1) {
			if (!boost_task_alive) {
				boost_task = kthread_create (
						cpufreq_wrexy_boost_task,
						NULL,
						"wrexy_input_boost_task"
				);
				...
				pr_info("[wrexy]:" "%s input boost task created\n", __func__);
				sched_setscheduler_nocheck(boost_task, SCHED_FIFO, &param);
				get_task_struct(boost_task);
				boost_task_alive = true;
			}
#ifdef CONFIG_INPUT_MEDIATOR
			input_register_mediator_secondary(&wrexy_input_mediator_handler);
#else
...
#ifdef CONFIG_HAS_EARLYSUSPEND
			register_early_suspend(&wrexy_early_suspend_handler);
#endif
	...
	case CPUFREQ_GOV_LIMITS:
		...
		if (this_wrexy->cur_policy->cur > new_policy->max) {
			dprintk(WREXY_DEBUG_JUMPS,"CPUFREQ_GOV_LIMITS jumping to new max freq: %d\n",new_policy->max);
			__cpufreq_driver_target(this_wrexy->cur_policy,
					new_policy->max, CPUFREQ_RELATION_H);
		}
		else if (this_wrexy->cur_policy->cur < new_policy->min) {
			dprintk(WREXY_DEBUG_JUMPS,"CPUFREQ_GOV_LIMITS jumping to new min freq: %d\n",new_policy->min);
			__cpufreq_driver_target(this_wrexy->cur_policy,
	...
	case CPUFREQ_GOV_STOP:
		dbs_timer_exit(this_wrexy);

		mutex_lock(&dbs_mutex);
		this_wrexy->cur_policy = NULL;
		dbs_enable--;
		...
		if (!dbs_enable){
			...
			sysfs_remove_group(cpufreq_global_kobject, &wrexy_attr_group);
#ifdef CONFIG_INPUT_MEDIATOR
			input_unregister_mediator_secondary(&wrexy_input_mediator_handler);
#else
			input_unregister_handler(&dbs_input_handler);
#endif
#ifdef CONFIG_HAS_EARLYSUSPEND
			unregister_early_suspend(&wrexy_early_suspend_handler);
#endif
		}
...
static int __init cpufreq_wrexy_init(void) {
	unsigned int i;
	struct wrexy_info_s *this_wrexy;
	u64 wall;
	u64 idle_time;
	...
	wrexy_wq = alloc_workqueue("wrexy_wq", WQ_HIGHPRI, 0);
	if (!wrexy_wq) {
		printk(KERN_ERR "Failed to create wrexy_wq workqueue\n");
		return -EFAULT;
	}
	...
	{
		this_wrexy = &per_cpu(wrexy_info, i);
		this_wrexy->cur_policy = NULL;
		this_wrexy->ramp_dir = 0;
		this_wrexy->freq_change_time = 0;
		this_wrexy->cur_cpu_load = 0;
		mutex_init(&this_wrexy->timer_mutex);
	}

#ifdef CONFIG_HAS_EARLYSUSPEND
	wrexy_early_suspend_handler.suspend = wrexy_early_suspend;
	wrexy_early_suspend_handler.resume = wrexy_late_resume;
	wrexy_early_suspend_handler.level = EARLY_SUSPEND_LEVEL_DISABLE_FB + 100;
#endif
...
fs_initcall(cpufreq_wrexy_init);
#else
module_init(cpufreq_wrexy_init);
#endif

static void __exit cpufreq_wrexy_exit(void) {
	unsigned int i;
	struct wrexy_info_s *this_wrexy;
	...
	{
		this_wrexy = &per_cpu(wrexy_info, i);
		mutex_destroy(&this_wrexy->timer_mutex);
	}
	destroy_workqueue(wrexy_wq);
}

module_exit(cpufreq_wrexy_exit);

MODULE_AUTHOR("jthatch12");
MODULE_DESCRIPTION("'cpufreq_wrexy' - A smart cpufreq governor");
MODULE_LICENSE("GPL");
